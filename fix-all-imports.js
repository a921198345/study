...
async function* walk(dir) {
  ...
  for (const dirent of dirents) {
    ...
    if (dirent.isDirectory()) {
      if (!dirent.name.startsWith('node_modules') && !dirent.name.startsWith('.git')) {
        yield* walk(res);
      }
    } else {
      yield res;
...
async function getRelativePath(filePath, importPath) {
  ...
  if (importPath.startsWith('@/')) {
    // 目标路径是importPath去掉@/
    const targetPath = path.join(projectRoot, importPath.slice(2));
    // 计算从当前文件目录到目标路径的相对路径
    let newImportPath = path.relative(fileDir, targetPath);
...
async function main() {
  ...
  console.log('Starting to fix imports in all files...');
